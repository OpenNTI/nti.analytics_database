...
class DynamicFriendsListMixin(object):
    ...
    @declared_attr
    def dfl_id(self):
        return Column('dfl_id', Integer, ForeignKey("DynamicFriendsListsCreated.dfl_id"),
...
class FriendsListMixin(object):
    ...
    @declared_attr
    def friends_list_id(self):
        return Column('friends_list_id', Integer, ForeignKey("FriendsListsCreated.friends_list_id"),
                      nullable=False, index=True)
...
class ChatsInitiated(Base, BaseTableMixin):
    ...
    chat_ds_id = Column('chat_ds_id', INTID_COLUMN_TYPE,
                        index=True, nullable=True, autoincrement=False)

    chat_id = Column('chat_id', Integer, Sequence('chat_seq'),
...
class DynamicFriendsListsCreated(Base, BaseTableMixin, DeletedMixin):
    ...
    dfl_ds_id = Column('dfl_ds_id', INTID_COLUMN_TYPE,
                       index=True, nullable=True, autoincrement=False)

    dfl_id = Column('dfl_id', Integer, Sequence('dfl_seq'),
                    index=True, nullable=False, primary_key=True)
...
class FriendsListsCreated(Base, BaseTableMixin, DeletedMixin):
    ...
    friends_list_ds_id = Column('friends_list_ds_id', INTID_COLUMN_TYPE,
                                index=True, nullable=True, autoincrement=False)

    friends_list_id = Column('friends_list_id', Integer, Sequence('friends_list_seq'),
                             index=True, nullable=False, primary_key=True)
