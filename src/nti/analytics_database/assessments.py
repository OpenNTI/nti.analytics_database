...
class AssignmentIdMixin(object):
    ...
    def assignment_id(self):
        return Column('assignment_id', NTIID_COLUMN_TYPE,
...
class DetailMixin(TimeLengthMixin):
    ...
    @declared_attr
    def question_part_id(self):
        return Column('question_part_id', INTID_COLUMN_TYPE, nullable=False,
                      autoincrement=False, index=True)

    @declared_attr
    def submission(self):
        # Null if left blank
        # (Freeform|MapEntry|Index|List)
        return Column('submission', Text, nullable=True)
...
class GradeMixin(object):
    ...
    @declared_attr
    def grader(self):
        return Column('grader', Integer, ForeignKey("Users.user_id"),
...
class AssignmentDetails(Base, DetailMixin, AssignmentSubmissionMixin):
    ...
    assignment_details_id = Column('assignment_details_id',
                                   ...
                                   primary_key=True)
    grade = relationship('AssignmentDetailGrades',
...
class AssignmentGrades(Base, AssignmentSubmissionMixin, GradeMixin):
    ...
    grade_id = Column('grade_id', Integer,
                      Sequence('assignment_grade_id_seq'),
...
class AssignmentDetailGrades(Base, GradeDetailMixin, AssignmentSubmissionMixin):
    ...
    question_part_id = Column('question_part_id', INTID_COLUMN_TYPE,
...
class AssignmentFeedback(Base, AssignmentSubmissionMixin, DeletedMixin):
    ...
    feedback_ds_id = Column('feedback_ds_id', INTID_COLUMN_TYPE,
                            nullable=True, unique=False)

    feedback_id = Column('feedback_id', Integer,
                         Sequence('feedback_id_seq'),
                         primary_key=True)
    ...
    grade_id = Column('grade_id', Integer,
                      ForeignKey("AssignmentGrades.grade_id"),
                      nullable=False)

    _file_mime_types = relationship('FeedbackUserFileUploadMimeTypes',
                                    lazy="select")
