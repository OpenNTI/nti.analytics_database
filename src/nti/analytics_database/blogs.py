...
class BlogMixin(object):
    ...
    @declared_attr
    def blog_id(self):
        return Column('blog_id', Integer, ForeignKey("BlogsCreated.blog_id"),
                      nullable=False, index=True)

    @declared_attr
    def _blog_record(self):
        return relationship('BlogsCreated', lazy="select",
                            foreign_keys=[self.blog_id])
...
class BlogsCreated(Base, BaseTableMixin, DeletedMixin, RatingsMixin):
    ...
    blog_ds_id = Column('blog_ds_id', INTID_COLUMN_TYPE,
                        nullable=True, autoincrement=False)

    blog_length = Column('blog_length', Integer,
                         nullable=True, autoincrement=False)

    blog_id = Column('blog_id', Integer, Sequence('blog_seq'),
...
class BlogCommentsCreated(Base, CommentsMixin, BlogMixin, RatingsMixin):
    ...
    _file_mime_types = relationship('BlogCommentsUserFileUploadMimeTypes', 
                                    lazy="select")
